...
public partial class CrewCard : Button
{
    ...
    public void Build(string portraitPath)
    {
        ...
        _focusBadge.Position = new Vector2(222, 10);
        _focusBadge.Size = new Vector2(51, 14);
        ...
        AddChild(_vitals);
        _status = TacticalUi.Label("", 12, "afc1c5");
    ...
    public void Synchronize(ActorObservation actor, bool selected, bool focused, string current, string pending, string target, Color color, int number)
    {
        ...
        TooltipText = $"{number:00} {actor.DisplayName} · {actor.Loadout?.WeaponName}\n{current}{(target.Length > 0 ? $" → {target}" : "")}"
